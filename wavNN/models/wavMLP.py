...
class WavMLP(nn.Module):
    def __init__(
        ...
        self.wavelet = lambda x: pywt.wavedec2(x, "db1", level=level).flatten()

        int(np.rint(in_channels / 2))
        in_channels = in_channels // 2**level
        ...
        self.tail = nn.Softmax(out_channels) if tail else None
    ...
    def forward(self, x):
        ...
        # stack the outputs
        concat = torch.stack([channel_1, channel_2, channel_3])
